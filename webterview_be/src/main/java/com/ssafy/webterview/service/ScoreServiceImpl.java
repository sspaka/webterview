...
public class ScoreServiceImpl implements ScoreService {
    ...
    public List<EvaluationDto> saveQuestion(int groupNo, MultipartFile file) throws Exception {
        ...
        evaluationList.add(new Evaluation(null, group, "특이사항", 3));
    ...
    @Override
    @Transactional
    public void saveScoreAndText(Map<String, Object> map) throws Exception {
        Applicant applicant = applicantRepository.getReferenceById((int) map.get("applicantNo"));
		Rater rater = raterRepository.getReferenceById((int) map.get("Rater"));

        for (Map.Entry<String, String> entry : new HashMap<>((Map<String,String>)map.get("evalList")).entrySet()) {
            Evaluation eval = evaluationRepository.getReferenceById(Integer.parseInt(entry.getKey()));
            if(eval.getEvaluationType()==3){
				gradeRepository.save(new Grade(null,rater,applicant,eval,null,entry.getValue()));
			}else{
				gradeRepository.save(new Grade(null, rater, applicant, eval, Integer.parseInt(entry.getValue()),null));
			}
        }
    }
    ...
    @Override
    public List<String> getGradeText(int applicantNo){
        return gradeRepository.getGradeText(applicantNo);
    }
