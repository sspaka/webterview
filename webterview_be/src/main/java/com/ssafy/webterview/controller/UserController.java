...
//
	...
	@PostMapping("/overlap")
	public ResponseEntity<Map<String, Object>> overlap(@RequestBody @ApiParam(value="입력한 이메일", required=true) UserDto userDto) {
		Map<String, Object> resultMap = new HashMap<>();
		HttpStatus status = HttpStatus.ACCEPTED;

		try {
			if(userService.mailOverlap(userDto.getUserEmail()) == null) {
				resultMap.put("message", "이메일 중복 x");
//				status = HttpStatus.ACCEPTED;
			}
			else {
				logger.error("이메일 중복 : {}");
				resultMap.put("message", "이메일 중복");
				status = HttpStatus.ACCEPTED;
			}
		} catch (Exception e) {
...
//				status = HttpStatus.ACCEPTED;
	...
	@PutMapping("/modify")
	public ResponseEntity<Map<String, Object>> modify(@RequestBody @ApiParam(value="수정할 정보를 담은 User객체", required=true) UserDto userDto, HttpServletRequest request)
			throws Exception {
		Map<String, Object> resultMap = new HashMap<>();
		HttpStatus status = HttpStatus.ACCEPTED;
		...
		if (jwtService.isUsable(request.getHeader("access-token"))) {
			logger.info("사용 가능한 토큰!!!");
			try {
				UserDto userdto = userService.modify(userDto);
...
//	
	...
	public ResponseEntity<Map<String, Object>> findEmail(@RequestBody @ApiParam(value="name과 phone 정보", required=true) Map<String, String> map, HttpServletRequest request)
		...
		try {
			UserDto find = userService.findEmail(map.get("name"),map.get("phone"));
			if(find != null){
				...
				resultMap.put("message", "해당하는 정보의 이메일이 존재하지 않거나, 이름이 일치하지 않습니다.");
				status = HttpStatus.ACCEPTED;
			}
		} catch (Exception e) {
			logger.error("정보수정 실패 : {}", e);
...
//
	...
	public ResponseEntity<Map<String, Object>> matchPw(@RequestBody @ApiParam(value="email, pw 정보", required=true) Map<String, String> map, HttpServletRequest request)
		...
		if(userService.matchPw(map.get("email"), map.get("pw"))) {
			...
			resultMap.put("message", "비밀번호 일치 x");
		}
